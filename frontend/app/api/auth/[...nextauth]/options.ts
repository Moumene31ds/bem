  ...
  providers: [
    CredentialsProvider({
      ...
      credentials: {
        ...
        headgear: { label: "Headgear", type: "text", placeholder: "cap" },
      },
        ...
        try {
          ...
          const user = await prisma.user.create({
            data: {
              ...
              headgear: credentials.headgear || "cap",
            },
          });
  ...
  callbacks: {
    async jwt({ token, user }) {
      if (user) {
        ...
        token.headgear = (user as any).headgear;
      }
      return token;
    },
    async session({ session, token }) {
      if (session.user) {
        ...
        (session.user as any).headgear = token.headgear;
      }
      return session;
    },
  },
